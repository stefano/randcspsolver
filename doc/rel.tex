...
L'algoritmo Branch\&Bound di ricerca \'e implementato in maniera ricorsiva
con l'aiuto di un ciclo while interno, che lavora unicamente sul dominio
corrente rimuovendone il valore scelto ad ogni iterazione.
Ad ogni nodo raggiunto si calcola la funzione euristica o quella obbiettivo nel caso si tratti
di una foglia dell'albero di ricorsione; se la funzione obbiettivo d\'a un
risultato positivo\footnote{Per positivo si intende un valore superiore, poich\'e
si considera il caso in cui si intende massimizzare il risultato.} 
rispetto al bound attuale allora l'algoritmo viene
richiamato al livello successivo, altrimenti si passa al valore successivo
della variabile corrente o eventualmente a quella precedente.
Nel caso si stia analizzando una foglia dell'albero, e quindi a tutte
...
una soluzione valida; 
 \item setSol(): imposta la soluzione corrente come quella migliore raggiunta fin'ora.
\end{itemize}
...
I valori vengono scelti in ordine crescente cos\'i come sono memorizzati
...
Poich\'e la scelta del valore avviene modificando i domini delle variabili
\'e necessario provvedere al ripristino di questi per permettere il corretto
avanzamento dell'algoritmo.
Il dominio della variabile corrente viene copiato all'inizio della ricorsione
mentre la copia di tutti i domini viene effettuata all'entrata nel ciclo while,
tramite il metodo \textit{copy()}.
Alla fine di ogni iterazione viene ripristinato il dominio della variabile
corrente e quelli delle variabili successivi con il metodo
\textit{setDomain()}, 
in modo da permettere alle sucessive iterazioni/ricorsioni di agire sui domini
completi.
...
\begin{itemize}
 \item -n: (int) imposta il numero di variabili rappresentate dal problema;
 \item -l: (int) imposta la caridnait\'a dei domini delle varibili;
 \item -d: (float) imposta la densit\'a dei vincoli;
 \item -s: (float) imposta la strettezza dei vincoli;
 \item -ac: indica l'utilizzo dell'algoritmo AC-1.
\end{itemize}
